...
## Algorithm
1.Import required libraries (pandas, numpy, sklearn RandomForestRegressor, metrics).

2.Load environmental sensor dataset and preprocess it (handle missing values, format date features).

3.Select input features (humidity, pressure, wind speed, date info) and targets (temperature, PM2.5, energy).

4.Split dataset into training and testing sets and apply Random Forest Regressor model.

5.Train model, predict outputs, and evaluate using R² score, MAE, RMSE, and cross-validation.

## Program:
```
Program to implement the Random Forest Algorithm to predict daily temperature , PM2.5 pollution level and Energy based on environmental sensor data.
Developed by: Hemapriya
